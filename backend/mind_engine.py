...
целое от 1 до 30), за который проверка возможна.

Проверка обязана опираться на наблюдение в мире, которое может сделать оператор:
что он увидит, замерит или зафиксирует, и какой исход подтвердит прогноз, а
какой опровергнет. Запрещено проверять прогноз собственной памятью и её
производными: «извлечь выборку эпизодов», «провести серию извлечений»,
«мониторить консолидации», «при достаточно большом объёме памяти появится
эпизод». Такая проверка подтверждается всегда и потому ничего не значит — она
описывает не мир, а перебор уже накопленного. Если способа наблюдения в мире не
придумывается, верни prediction = null: отсутствие прогноза честнее
неопровержимого. Не создавай связь только потому, что две концепции были выбраны вместе.
...
Поле operator_request заполняй, только если своей памяти и оснований не хватает
принципиально и нужен внешний факт: наблюдение, замер, фрагмент источника.
Формулируй его как просьбу о конкретном свидетельстве, а не как тему для
размышления, и назови, что именно подтвердит или опровергнет гипотезу.
Проси то, что оператор способен наблюдать в мире. Не проси извлечь эпизоды
собственной памяти, увеличить выборку или дождаться консолидации — это ты
делаешь сам, и внешним свидетельством оно не является. Если
своих данных достаточно, верни null: просьба без нужды тратит единственный
доступный тебе канал наружу.
...
{{
  ...
  "operator_request": "<что именно нужно получить извне, чтобы решить вопрос, либо null>",
